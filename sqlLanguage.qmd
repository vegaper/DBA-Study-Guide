...
``` sql
SELECT SUM(CASE WHEN SECTOR 0 'Technology' THEN Revenue END) AS Revenue_Technology,
...
- **Mínimo de cotización:** ¿Cuál sería el día con la cotización más baja dentro de una tendencia alcista (campo `High`) en 2018?
  ...
  ``` sql
  SELECT * FROM bitcoin_daily_rates_formatdate
  WHERE 
  ...
  ``` sql
  SELECT date, min(high) as cotización_mínima FROM bitcoin_daily_rates_formatdate
  WHERE 
  date BETWEEN '2018-00-00' AND '2018-99-99'
  ```
...
WHERE homeworld != 'NA' and eye_color != 'unknown'
...
, COUNT(*)
FROM LoanData
GROUP BY personal_status, class
```
...
``` sql
SELECT  movie_title, gender, imdb_score, actor_1_facebook,title_year FROM imdb_movies
WHERE   
...
``` sql
SELECT movie_title, gender, budget FROM imdb_movies
ORDER BY budget DESC 
LIMIT 20
```

``` sql
SELECT movie_title, gender, gross FROM imdb_movies
ORDER BY gross DESC 
LIMIT 20
```
...
Usando la tabla fortune: Clasifica las empresas en tres categorías según su número de empleados (Employees):

Pequeña: Menos de 10.000 empleados. Mediana: Entre 10.000 y 50.000 empleados. Grande: Más de 50.000 empleados.
...
Usando la tabla jamesbond: ¿Cuales son las 5 películas de James Bond con mayor margen sobre ingresos (% del beneficio)? Indicar película, director, actor y margen
...
Si repartimos la tarta de esos 448,8 M€ recaudados:Tan solo el 1,56% de la recaudación sirvió para cubrir los costes de producción ($7 / 448,8$). El 98,44% restante ($441,8 / 448,8$) es beneficio puro. En resumen: De cada 100 € que la gente se gastó en el cine para ver la película, 1,56 € sirvieron para pagar el presupuesto y 98,44 € fueron ganancia neta. De ahí sale ese 98,44%.
...
------------------------------------------------------------------------
Usando la tabla LoanData
Calcular la cantidad de créditos concedidos según hombres y mujeres, clasificados por tres grupos de edad (menos de 30 años, de 30 a 40 años y más de 40 años), 
que tienen un coche en propiedad y viven en la casa propia.
