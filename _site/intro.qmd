...
1\. **La porción de Red:** Identifica a la red específica a la que pertenece el equipo .

2\. **La porción de Host:** Identifica al propio dispositivo (el equipo) dentro de esa red concreta .
...
1\. **Dirección de Red:** Es la primera IP, reservada para identificar la red y no la puede llevar ningún equipo .

2\. **Dirección de Broadcast:** Es la última IP disponible (ej. `192.168.1.255`), común a todos los dispositivos . Si se envía información a ella, la reciben todos los equipos .

3\. **Puerta de Enlace Predeterminada:** Generalmente se le da la primera dirección disponible (ej. `192.168.1.1`) y corresponde al router. Esta configuración sólo será necesaria para salir fuera de la red.
...
-   **Ips Disponibles:** 2 elevado al número de bits que hay disponibles para los dispositivos. En nuestro caso 8 = 256
...
-   **Análisis de la Máscara:** Una máscara **`/23`** toma los primeros 16 bits completos (los dos primeros bloques), y **7 bits del tercer bloque** (16+7=23). Esto significa que para los hosts nos quedan 9 bits libres (32−23=9). Tener 9 bits disponibles nos da ($2^9$)=512 direcciones para dispositivos. La máscara quedaría como 11111111.11111111.11111110.00000000
...
-   **Ips disponibles:** Tenemos disponibles 9 bits después de restar los de la red, así que ($2^9$) = 512
...
-   **Análisis:** Una máscara **/25** significa que tomamos los tres primeros bloques completos (24 bits) y le "robamos" 1 bit al último bloque (24+1=25). En binario, la máscara de ese bloque de bits queda 10000000, es decir, el primero queda reservado para la identificación de la red. Ese número en decimal es 128.
...
-   **Direcciones Ip disponibles:** ($2^7$) =128
...
### Ver los valores con la consola de comandos.

Para abrir la consola de comandos en la barra de búsqueda de windows escribimos cmd

Dentro de la consola usamos el comando `ipconfig` y nos dará una descripción breve donde tenemos casi todos los valores que necesitamos saber sobre la configuración ip de nuestro equipo

![](images/Picture1.jpg)

o podemos usar `ipconfig /all` para conseguir más información, como la MAC que aparece como *Dirección física*.

La información que nos aparece en la consola nos da nuestra Ip Privada, para saber la ip Pública, con la que se nos ve fuera de nuestra red tenemos que usar un navegador. Hay distintas páginas que nos la dan como whatismyipaddress.com o cual-es-mi-ip.net ,por ejemplo.

## Modelo TCP/IP y el Viaje de los Datos
...
\*\* Consejos de Gestión y Seguridad para el DBA:\*\*
...
Podemos usar la consola de comandos para verficasr qué conexiones hay en un puerto. Por ejemplo, para comprobar las conexiones al puerto 443 usamos el comando `netstat -an | find ":443"`

![](images/Picture2.jpg)

Esto nos informa del protocolo, la dirección y el puerto de la petición y la dirección y el puerto de destino.
